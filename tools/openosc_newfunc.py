...
def rtd_parse_options():
    ...
    parser.add_argument("-p", "--prototype", help="the function prototype")
    parser.add_argument(
        "-i",
        "--inputfile",
    ...
    global g_magic_word
    if args.magicword:
        g_magic_word = get_wrap_magic_word(int(args.magicword, 0),
                                           g_magic_alignment)
...
def generate_openosc_redefine_map_code_new(prototype):
    ...
    print(HDR1)
    print("Now generating OpenOSC Macro Redefine mapping code for: " +
          funcname)
    print(HDR1)
    magic_str = "_CASE3"
    ...
    func_code = generate_osc_mapping_func_body_with_va_arg_pack(
        funcname, magic_str)
...
def generate_openosc_redefine_map_code(prototype):
    ...
    """
    (retval, funcname, params) = get_func_params_from_prototype(prototype)
    (dest_type, dest_param, copylen, src_type,
     src_param) = analyze_func_params(params)
    ...
    code += "  OSC_THROW\n{\n"
    if not dest_param:
        code += ("    return (" + funcname_up + "_CASE3 __" + funcname +
                 "_to_buf(OPENOSC_USE_FORTIFY_LEVEL - 1, " + comma_params +
                 "));\n")
        code += "}\n\n"
    ...
    if "wchar_t" in dest_type:
        byte_sz = "_sz_bytes"
        code += ("    size_t _sz_bytes = __builtin_object_size(" + dest_param +
                 ", " + bosc_check_type + ");\n")
        code += "    size_t _sz = _sz_bytes / sizeof(wchar_t);\n"
    else:
        code += ("    size_t _sz = __builtin_object_size(" + dest_param +
                 ", " + bosc_check_type + ");\n")
    if copylen:
        code += "    int is_len_constant = __builtin_constant_p(" + \
            copylen + ");\n"
        code += ("    return (((" + byte_sz + " != (size_t) -1) && (" +
                 byte_sz + " != 0))\n")
        code += "            ? (is_len_constant\n"
        code += "               ? ((_sz >= " + copylen + ")\n"
        code += ("                  ? (" + funcname_up + "_CASE1 " + funcname +
                 "(" + comma_params + "))\n")
        code += ("                  : (" + funcname_up + "_CASE2 OSC_ASSERT_" +
                 funcname + "()))\n")
        code += ("               : (" + funcname_up +
                 "_CASE3 OSC_RUNTIME_CHECK_" + funcname + "()))\n")
        code += ("            : (" + funcname_up + "_CASE4 " + funcname + "(" +
                 comma_params + ")));\n")
    else:
        code += ("    return (((" + byte_sz + " != (size_t) -1) && (" +
                 byte_sz + " != 0))\n")
...
def generate_openosc_redirect_map_code_new(prototype):
    ...
    (retval, funcname, params) = get_func_params_from_prototype(prototype)
    print(HDR1)
    print("Now generating OpenOSC ASM-Label Redirect Mapping code for: " +
          funcname)
    print(HDR1)
    magic_str = "_CASE3"
    va_args_code = generate_va_args_redefine_code(funcname, params, magic_str)
...
def generate_openosc_redirect_map_code(prototype):
    ...
    code += "\n__" + funcname + "_to_buf(size_t dest_len, " + params + ");\n"
    code += "extern " + retval + \
        " __REDIRECT_NTH (__openosc_" + funcname + "_alias,\n"
    code += "                              (" + \
        params + "), " + funcname + ");\n"
    code += "extern " + retval + \
        " __REDIRECT_NTH (openosc_" + funcname + "_chk_warn,\n"
    code += ("                              (size_t dest_len, " + params +
             "), __" + funcname + "_to_buf)\n")
    code += (
        '       __warnattr ("' + funcname +
        ' caller with bigger length than size of destination buffer");\n\n')
    ...
    if copylen:
        ...
        code += "            ? (is_len_constant\n"
        code += "               ? ((_sz >= " + copylen + ")\n"
        code += ("                  ? (" + funcname_up + "_CASE1 __openosc_" +
                 funcname + "_alias(" + comma_params + "))\n")
        code += ("                  : (" + funcname_up + "_CASE2 openosc_" +
                 funcname + "_chk_warn(_sz, " + comma_params + ")))\n")
        code += ("               : (" + funcname_up + "_CASE3 __" + funcname +
                 "_to_buf(_sz, " + comma_params + ")))\n")
        code += ("            : (" + funcname_up + "_CASE4 __openosc_" +
                 funcname + "_alias(" + comma_params + ")));\n")
    else:
        code += ("    return (((" + byte_sz + " != (size_t) -1) && (" +
                 byte_sz + " != 0))\n")
...
def generate_osc_original_runtime_check_macro(prototype):
    ...
    (retval, funcname, params) = get_func_params_from_prototype(prototype)
    comma_params = get_comma_joined_param_names(params)
    return ("#define OSC_RUNTIME_CHECK_" + funcname + "()\t\t" + funcname +
            "(" + comma_params + ")\n")
...
def generate_osc_runtime_check_macro(prototype):
    ...
    (retval, funcname, params) = get_func_params_from_prototype(prototype)
    comma_params = get_comma_joined_param_names(params)
    return ("#define OSC_RUNTIME_CHECK_" + funcname + "()\t\t__" + funcname +
            "_to_buf(_sz, " + comma_params + ")\n")
...
def generate_osc_compiletime_check_macro(prototype, checktype):
    ...
    if checktype == "OSC_ASSERT_USE_ERR_ATTR":
        code += ('extern int __attribute__((error("Compiler Assertion: ' +
                 funcname + ' len will always overflow dst"))) \\\n')
        code += " osc_compile_check_" + funcname + "(void);\n"
        code += ("#define OSC_ASSERT_" + funcname +
                 "()  ({osc_compile_check_" + funcname + "(), (" + retval +
                 ")0;})\n\n")
    elif checktype == "OSC_ASSERT_USE_BUILTIN":
        code += ("#define OSC_ASSERT_" + funcname + "()\t\t__" + funcname +
                 "_chk(" + comma_params + ", _sz)\n")
    elif checktype == "OSC_ASSERT_USE_RUNTIME_CHK":
        code += ("#define OSC_ASSERT_" + funcname + "()\t\t__" + funcname +
                 "_to_buf(_sz, " + comma_params + ")\n")
    elif checktype == "OSC_ASSERT_USE_NONE":
        code += ("#define OSC_ASSERT_" + funcname + "()\t\t" + funcname + "(" +
                 comma_params + ")\n")
    return code
...
def generate_nomap_magic_code_for_func(funcname, int_magic=0, macrotype=""):
    ...
    :returns the generated code
    """
    print("Now generating OpenOSC nomap magic code for function " + funcname +
          " with MAGIC word: " + hex(int_magic) + " for macro-type: " +
          macrotype)
    ...
    if macrotype == "MAGIC":
        if args.magictype == "dwarf_loc":
            ...
            code += get_byte_array_of_magic_word(
                int_magic) + '\\n" OSC_JUMPLABEL\n'
        int_magic += 1
        update_global_magic_word(int_magic)
    elif macrotype == "CASE":
        code += ("#define " + func_up + "_NOMAP_CASE" + "\t\t({asm(" +
                 func_up + "_NOMAP_MAGIC" + ");}),\n")
    else:
...
def generate_map_magic_code_for_func(funcname, int_magic=0, macrotype=""):
    ...
    :returns the generated code
    """
    print("Now generating OpenOSC map magic code for function " + funcname +
          " with MAGIC word: " + hex(int_magic) + " for macro-type: " +
          macrotype)
    ...
    if macrotype == "MAGIC":
        for i in range(4):
            if args.magictype == "dwarf_loc":
                code += "#define " + func_up + "_MAGIC" + str(
                    i + 1) + '\t\t".loc 1 '
                code += str(int_magic - 1) + '"\n'
            else:
                code += ("#define " + func_up + "_MAGIC" + str(i + 1) +
                         '\t\tOSC_JUMPOVER ".byte ')
                code += get_byte_array_of_magic_word(
                    int_magic) + '\\n" OSC_JUMPLABEL\n'
            int_magic += 1
...
def generate_gnu_redirect_func_declaration(retval, funcname, params):
    ...
    """
    code = ""
    code += ("extern " + retval + " __REDIRECT_NTH (" +
             get_gnu_redirect_function_name(funcname) + ",\n")
...
def replace_flag_slen_nofmt_params(params):
    ...
    (dest_type, dest_param) = get_dest_param(params)
    # param_list = get_param_list_from_params_string(params)
    bos_dest = "__builtin_object_size(" + \
        dest_param + ", OSC_OBJECT_SIZE_CHECK_1)"
    names = get_param_name_2less_list(params)
    newnames = []
    for name in names:
...
def generate_va_args_redefine_code0(funcname, params):
    ...
    """
    code = ""
    code += ("#define " + funcname + "(" +
             get_comma_joined_param_names(params) + ") \\\n")
    code += " (" + funcname.upper() + "_NOMAP_CASE "
    code += (funcname + "(" + ", ".join(get_param_name_2less_list(params)) +
             ", __VA_ARGS__))\n")
    return code
...
def generate_va_args_redefine_code(funcname, params, magic_str="_NOMAP_CASE"):
    ...
    (retval, chk_funcname,
     chk_params) = get_func_params_from_prototype(glibc_chk_prototype)
...
def generate_gnu_redirect_func_body_with_va_arg_pack(funcname, magic_str):
    ...
    code += "__fortify_function " + retval + "\n"
    code += "__NTH (" + funcname + "(" + delete_flag_slen_params(
        chk_params) + "))\n{\n"
...
def generate_osc_mapping_func_body_new(retval,
                                       funcname,
                                       params,
                                       magic_str="_NOMAP_CASE"):
    """
    Generate both the declaration of the alias function and the fortify function body
    of the function that calls the NOMAP alias function for ASM-Label redirect mechanism.
...
def generate_osc_mapping_func_body_with_va_arg_pack(funcname, magic_str):
    ...
    code += (retval + "\nopenosc_" + funcname + " (" +
             delete_flag_slen_params(chk_params) + ")")
    code += "\n{\n"
    if has_dest_param(funcname):
        # prototype = get_func_prototype_from_man(funcname)
        # (retval, funcname, params) = get_func_params_from_prototype(prototype)
        (dest_type, dest_param, copylen, src_type,
         src_param) = analyze_func_params(chk_params)
        if "char" in dest_type or "wchar_t" in dest_type:
            bosc_check_type = "OSC_OBJECT_SIZE_CHECK_1"
...
def generate_osc_mapping_func_body(retval, funcname, params):
    ...
    code = ""
    if "..." not in params:
        code += ("extern " + retval + " __" + funcname +
                 "_to_buf (size_t dest_len, " + params + ");\n\n")
    code += "static inline __attribute__ ((always_inline))\n"
...
def generate_osc_nomap_func_redirect(retval, funcname, params):
    ...
    print(HDR1)
    print(
        "Now generating OpenOSC nomap code for ASM-label redirect method for: "
        + funcname)
    print(HDR1)
    code = "\n"
    code += "/* Mapping for " + funcname + " */\n\n"
...
def generate_osc_nomap_func_redefine(retval, funcname, params):
    ...
    """
    print(HDR1)
    print("Now generating OpenOSC nomap code for macro define method for: " +
          funcname)
    print(HDR1)
    code = "\n"
    code += "/* Mapping for " + funcname + " */\n\n"
...
def generate_functions_runtime_check_macro_code(functions):
    ...
    "returns the generated code
    """
    print("Now generating runtime check MACRO code for " +
          str(len(functions)) + " functions.\n")
    code = ""
...
def generate_functions_compiletime_check_macro_code(functions):
    ...
    "returns the generated code
    """
    print("Now generating compile time check MACRO code for " +
          str(len(functions)) + " functions.\n")
    code = ""
    ...
    for func in functions:
        ...
        if not prototype or "..." in prototype:
            continue
        code += generate_osc_compiletime_check_macro(
            prototype, "OSC_ASSERT_USE_ERR_ATTR")
    code += "\n#elif (OSC_COMPILE_CHK == OSC_ASSERT_USE_BUILTIN)\n\n"
    for func in functions:
        prototype = get_func_prototype_from_man(func)
        if not prototype or "..." in prototype:
            continue
        code += generate_osc_compiletime_check_macro(prototype,
    ...
    for func in functions:
        ...
        if not prototype or "..." in prototype:
            continue
        code += generate_osc_compiletime_check_macro(
            prototype, "OSC_ASSERT_USE_RUNTIME_CHK")
...
def generate_functions_nomap_magic_macro_code(functions, int_magic=0):
    ...
    code += "\n#ifdef OSC_METRIC_FEATURE_ENABLED\n\n"
    for func in functions:
        code += generate_nomap_magic_code_for_func(func,
                                                   g_magic_words[func] + 1,
                                                   "MAGIC")
    code += "\n"
    for func in functions:
        code += generate_nomap_magic_code_for_func(func,
                                                   g_magic_words[func] + 2,
                                                   "CASE")
    code += "\n#else  /* !OSC_METRIC_FEATURE_ENABLED */\n\n"
    for func in functions:
        code += generate_nomap_magic_code_for_func(func,
...
def generate_openosc_file_start_code(functions):
    ...
    openosc_write_filename("openosc_fortify_nomap.h",
                           openosc_nomap_mm_template)
...
def generate_fortify_functions_real_code(functions):
    ...
    for func in functions:
        ...
        if not prototype:
            ...
            print("No function prototype found for " + func +
                  ", Please specify prototype via -p option.")
            print("")
            continue
        ...
        if False:
            ...
            generate_osc_nomap_code(
                prototype,
                get_wrap_magic_word(g_magic_word, g_magic_alignment) + 1)
...
def generate_openosc_nomap_test_code(prototype, codetype=""):
    ...
    if codetype == "test":
        ...
        code += "    int mylen = 8;\n"
        code += "    " + dest_chartype + \
            " mydstbuf[15 * " + dest_charsize + "];\n"
        code += ("    " + dest_chartype + " **pmydstbuf = (" + dest_chartype +
                 " **)(&mydstbuf);\n")
        code += ("    const " + src_chartype + " mysrcbuf[20] = " +
                 literal_prefix + '"I am the source";\n')
        code += ("    const " + src_chartype + " **pmysrcbuf = (const " +
                 src_chartype + " **)(&mysrcbuf);\n")
        ...
        if "mbstate_t" in params:
            ...
            code += "    size_t nms = 1;\n    size_t nwc = 1;\n"
        invoke_params = ", ".join(
            get_default_value_list_for_parameters(params))
        code += '    printf("Line %d, func %s, mylen is: %d\\n", __LINE__, __FUNCTION__, mylen);\n'
        code += ("    " + funcname + "(" + invoke_params +
                 ");          /* function invocation */\n")
...
def generate_openosc_test_code(prototype, codetype=""):
    ...
    if dest_param and copylen and src_param:
        ...
        if codetype == "test":
            ...
            code += "    int mylen = 8;\n"
            code += ("    " + dest_chartype + " " + dest_param + "[5 * " +
                     dest_charsize + "];\n")
            code += ("    const " + src_chartype + " " + src_buf + "[20] = " +
                     literal_prefix + '"I am the source";\n')
            if "**" in src_type:
                code += ("    " + src_type + " " + src_param + " = (" +
                         src_type + ")(&" + src_buf + ");\n")
            code += "    size_t " + copylen + " = 4;\n"
            code += ("    " + funcname + "(" + comma_params +
                     ");          /* case 1 */\n")
            code += (
            ...
            code += (
                '    printf("Line %d, func %s, copylen is: %d\\n", __LINE__, __FUNCTION__, '
                + copylen + ");\n")
            code += ("    " + copylen + " = (" + src_buf + "[18]-" +
                     dest_param + "[2])/" + downfactor + ";\n")
            code += ("    " + funcname + "(" + comma_params +
                     ");          /* case 3 */\n")
            code += (
                '    printf("Line %d, func %s, copylen is: %d\\n", __LINE__, __FUNCTION__, '
                + copylen + ");\n")
            code += ("    " + copylen + " = (*((char *)" + "&mylen - " +
                     src_buf + "[18] * 8))/" + downfactor + ";\n")
            code += ("    " + funcname + "(" + comma_params +
                     ");          /* case 3 */\n")
            code += (
                '    printf("Line %d, func %s, copylen is: %d\\n", __LINE__, __FUNCTION__, '
                + copylen + ");\n")
            code += "    " + copylen + " = 1;\n"
            new_comma_params = comma_params.replace(
                dest_param + ",",
                "(" + dest_type + ")((char *)&mylen - " + dest_param +
                "[18] * 4),",
            )
            code += ("    " + funcname + "(" + new_comma_params +
                     ");          /* case 4 */\n")
            code += (
                '    printf("Line %d, func %s, copylen is: %d\\n", __LINE__, __FUNCTION__, '
                + copylen + ");\n")
            new_comma_params = comma_params.replace(
                dest_param + ",",
                "(" + dest_type +
                ")((char *)&mylen - rand() % OPENOSC_TEST_RAND_RANGE),",
            )
            code += ("    " + funcname + "(" + new_comma_params +
                     ");          /* case 4 */\n")
            code += (
...
def find_flag_param_index_in_params(params):
    ...
    for i in range(len(param_list)):
        param = param_list[i]
        (param_type, param_name) = get_type_name_from_param(param)
        if param_type == "int" and (param_name == "flag"
                                    or param_name == "flags"):
            return i
    return -1
...
def find_slen_param_index_in_params(params):
    ...
    for i in range(len(param_list)):
        param = param_list[i]
        (param_type, param_name) = get_type_name_from_param(param)
        if param_type == "size_t" and (param_name == "slen"
                                       or param_name == "s_len"):
            return i
    return -1
...
def generate_runtime_check_code2(prototype):
    ...
    if glibc_chk_funcname in glibc_print_chk_prototype_db:
        glibc_chk_prototype = glibc_print_chk_prototype_db[glibc_chk_funcname]
        code += "extern " + strip_semi_colon_in_prototype(
            glibc_chk_prototype) + ";\n\n"
    code += retval
    code += "\n__" + funcname + "_to_buf (size_t dest_len, " + params + ")"
    like_which = "like_memcpy"
    ...
    if glibc_chk_funcname in glibc_print_chk_prototype_db:
        glibc_chk_prototype = glibc_print_chk_prototype_db[glibc_chk_funcname]
        (glibc_retval, glibc_funcname,
         glibc_params) = get_func_params_from_prototype(glibc_chk_prototype)
        flag_index = find_flag_param_index_in_params(glibc_params)
        new_comma_params = insert_glibc_print_chk_flag_with_destlen(
            params, flag_index)
...
def get_dest_param(params):
    ...
    for token in tokens:
        ...
        if ("*" not in param_type or "const " in param_type
                or "src" in param_name or "FILE" in param_type
                or "**" in param_type):
            continue
...
def get_src_param(params):
    ...
    for token in tokens:
        ...
        if token == "...":
            continue
        (param_type, param_name) = get_type_name_from_param(token)
        if ("*" not in param_type or "format" in param_name
                or "dest" in param_name or "dst" in param_name
                or "FILE" in param_type):
            continue
    ...
    for line in lines:
        ...
        if "#include " == line[:9]:
            ...
            if ")" in line:  # end of prototype
                (retval, func,
                 params) = get_func_params_from_prototype(prototype)
                num_functions += 1
                functions.append(func)
                update_prototype_db(func, prototype, inc_lines, prototype_db,
                                    include_db)
                inc_lines = []
                prototype = ""
...
def get_all_functions():
    ...
    print("MAGIC word start: " + hex(g_magic_word) +
          " alignment size for function is: " + str(alignment_size))
    if not functions:
